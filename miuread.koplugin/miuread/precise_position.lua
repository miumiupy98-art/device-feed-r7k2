...
local function compact_compare_text(value)
    return tostring(value or ""):gsub("%s", ""):gsub("%*", "")
end

local function remote_local_chapter(record, wanted_uid)
    wanted_uid = tostring(wanted_uid or "")
    ...
    local explicit_uid = scalar(record and record.record and record.record.chapter_uid)
    if explicit_uid ~= "" and explicit_uid == wanted_uid and readable <= 1 then
        local row, row_index
        for index, item in ipairs(local_map) do
            if tostring(chapter_uid(item) or "") == wanted_uid then
                row, row_index = item, index
                break
            end
        end
        row, row_index = row or local_map[1] or {}, row_index or 1
        return row, row_index, true
    end
    for index, row in ipairs(local_map) do
        if type(row) == "table" and tostring(chapter_uid(row) or "") == wanted_uid then
...
local function result_xpointer(result, key)
    if type(result) ~= "table" then return nil end
    return result[key] or (key == "start" and result.pos0 or result.pos1)
end

local function search_hit_context(document, hit, candidate)
    local start_xp = result_xpointer(hit, "start")
    local end_xp = result_xpointer(hit, "end") or start_xp
    if not valid_xpointer(document, start_xp) then return false end
    local expected_before = compact_compare_text(candidate.context_before)
    local expected_after = compact_compare_text(candidate.context_after)
    if expected_before == "" and expected_after == "" then return false end

    local before = ""
    local before_xp = retreat_words(document, start_xp, 20)
    if before_xp then before = doc_text(document, before_xp, start_xp) or "" end
    local after = ""
    local after_xp = advance_words(document, end_xp, 20)
    if after_xp then after = doc_text(document, end_xp, after_xp) or "" end
    before = compact_compare_text(before)
    after = compact_compare_text(after)

    local before_ok = expected_before == ""
        or (#before >= #expected_before and before:sub(-#expected_before) == expected_before)
    local after_ok = expected_after == ""
        or (#after >= #expected_after and after:sub(1, #expected_after) == expected_after)
    return before_ok and after_ok
end

-- Locate a source-text anchor produced from a native WeRead cloud `co` inside
-- the already-open local EPUB. CREngine's enhanced text search handles text
-- split across formatting nodes and folded whitespace, so we do not need to
-- rebuild or rescan the EPUB source in Lua. This runs only when the user adopts
-- a cloud position; normal page turns and read reports never call it.
function M.findSourceAnchor(ui, record, source_anchor)
    source_anchor = type(source_anchor) == "table" and source_anchor or {}
    if type(record) ~= "table" then return nil, "record_missing" end
    local document = ui and ui.document or nil
    local toc = ui and ui.toc or nil
    if not document or type(document.findText) ~= "function" then
        return nil, "document_search_unavailable"
    end

    local uid = tostring(source_anchor.chapter_uid or "")
    local _, expected_toc, standalone = remote_local_chapter(record, uid)
    if not expected_toc then return nil, "remote_chapter_not_local" end
    local candidates = type(source_anchor.search_candidates) == "table"
        and source_anchor.search_candidates or {}
    if #candidates == 0 then return nil, "remote_source_candidates_missing" end

    local function chapter_hit(xp)
        if not valid_xpointer(document, xp) then return false end
        if standalone then return true end
        local index = toc_index_for_xpointer(toc, xp)
        return tonumber(index) == tonumber(expected_toc)
    end

    local total_hits = 0
    for _, candidate in ipairs(candidates) do
        local pattern = U.trim(tostring(candidate.text or ""))
        if pattern ~= "" then
            local ok, results = pcall(document.findText, document,
                pattern, -1, 0, true, nil, false, 16, 0x00FF)
            if ok and type(results) == "table" then
                local filtered = {}
                for _, hit in ipairs(results) do
                    local xp = result_xpointer(hit, "start")
                    if chapter_hit(xp) then filtered[#filtered + 1] = hit end
                end
                total_hits = total_hits + #filtered
                if #filtered == 1 then
                    local hit = filtered[1]
                    return {
                        xpointer = result_xpointer(hit, "start"),
                        end_xpointer = result_xpointer(hit, "end"),
                        chapter_uid = uid,
                        toc_index = expected_toc,
                        search_text = pattern,
                        search_chars = U.utf8_len(pattern),
                        search_hits = 1,
                        source_text_ratio = tonumber(source_anchor.source_text_ratio),
                        source = "native_cloud_text_anchor",
                        confidence = "exact_text",
                    }
                elseif #filtered > 1 then
                    local contextual = {}
                    for _, hit in ipairs(filtered) do
                        if search_hit_context(document, hit, candidate) then
                            contextual[#contextual + 1] = hit
                        end
                    end
                    if #contextual == 1 then
                        local hit = contextual[1]
                        return {
                            xpointer = result_xpointer(hit, "start"),
                            end_xpointer = result_xpointer(hit, "end"),
                            chapter_uid = uid,
    ...
    return nil, total_hits > 0 and "remote_source_anchor_ambiguous" or "remote_source_anchor_not_found"
end
