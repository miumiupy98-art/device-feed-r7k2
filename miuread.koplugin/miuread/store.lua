...
function Store:migrate()
    ...
    if schema<Config.SCHEMA then
        ...
        if schema<101 then
            ...
            local current=self:preferences()
            current.mode_intro={last_confirmed_mode="",confirmed_at=0}
            current.notices=type(current.notices)=="table" and current.notices or {}
