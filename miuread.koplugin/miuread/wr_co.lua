...
function M.toMap(map, co)
    ...
    for i = 1, #runes do
        if units == target then
            return {
                ...
                basis = "raw_xhtml_utf16",
                rune_boundary = i,
                codepoints_before = i - 1,
                utf16_extra = units - (i - 1),
                exact = true,
            }
        end
        local width = utf16_units(runes[i])
        if units + width > target then
            -- A valid Web Reader data-wr-co should always fall on a source
            -- character boundary. Be conservative if an old/broken cloud
            -- value lands inside a surrogate pair: snap to the rune start and
            -- surface that fact to the caller instead of inventing a byte index.
            return {
                co = target,
                basis = "raw_xhtml_utf16",
                rune_boundary = i,
    ...
    if units == target then
        return {
            co = target,
            basis = "raw_xhtml_utf16",
            rune_boundary = #runes + 1,
            codepoints_before = #runes,
            utf16_extra = units - #runes,
            exact = true,
        }
    end
