...
## 4.5.0-beta.18 - 2026-08-16
- 优化 阅读空闲时轻量预热当前章节评论索引，减少首次打开评论的 SQLite 冷启动卡顿；翻页与后台下载仍保持前台优先。
- 优化 封面自动下载失败加入 30 秒至 30 分钟递增退避，主页操作不再反复重置；手动刷新仍可立即重试。
- 优化 更新检查记忆上次成功线路并缩短清单线路超时；安装包下载与 SHA-256 校验策略保持不变。
- 修复 批注明确服务端拒绝不再自动无限重试；云端进度优先比较章节与 offset，百分比只在缺少精确坐标时参与冲突判断。
