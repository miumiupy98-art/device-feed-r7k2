...
- 优化 后台下载由“主页/阅读一操作就整任务暂停”改为短时让路：网络与普通章节继续低速推进，EPUB 打包、正文转换等重阶段等待前台安静后再执行。
- 修复 打开下载进度或连续主页操作后，`home_interaction` 暂停可能长期残留、必须取消再开始才能继续的问题。
- 稳定 页面切换等短暂暂停增加超时自愈；恢复回调异常时会自动清理过期暂停，同时保留真正休眠、评论弹窗和重资源保护。
- 保持 断点续传、锁屏后台下载、IPv4 自动建议、低内存安全休眠与 Reader 前台优先逻辑不变。
